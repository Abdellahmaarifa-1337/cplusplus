...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (_energyPoints <= 0 || _hitPoints <= 0)   
    {
        std::cout << "ClapTrap " << _name << " can't repaire!\n";
        return ;
    }
    _hitPoints += amount;
    _energyPoints -= 1;
    std::cout << "CLapTrap " << _name << " gets " << amount << " of hit points back!\n";
};
...
void ClapTrap::state(std::string st)
{
    ...
    std::cout << std::setfill(' ') << std::setw(50) <<" ClapTrap " + _name + " State after : " + st << " ";
    std::cout << std::setfill('*') << std::setw(5) << "" << "\n";
